...
use crate::muve::Move;
use crate::utils::{lsb, remove_lsb, PieceType, COLOR};
...
impl Board {
    ...
    pub fn refresh_accumulator(&mut self) {
        ...
        let my_index = self.nnue.feature_transformer.make_index(
            KING_IND,
            0,
            sq_my_perspective as usize,
            my_king_square as usize,
        );
        let opp_index = self.nnue.feature_transformer.make_index(
            KING_IND,
            0,
            sq_opp_perspective as usize,
            opp_king_square as usize,
        );

        self.nnue
            .feature_transformer
            .add_to_accumulator(my_index, &mut my_acc);
        self.nnue
            .feature_transformer
            .add_to_accumulator(opp_index, &mut opp_acc);
        self.nnue
            .feature_transformer
            .add_to_accumulator_psq(my_index, &mut my_psq_acc);
        self.nnue
            .feature_transformer
            .add_to_accumulator_psq(opp_index, &mut opp_psq_acc);
    ...
    fn update_specific_piece(
        ...
        while sq != 64 {
            ...
            self.nnue
                .feature_transformer
                .add_to_accumulator(my_index, my_acc);
            self.nnue
                .feature_transformer
                .add_to_accumulator(opp_index, opp_acc);
            self.nnue
                .feature_transformer
                .add_to_accumulator_psq(my_index, my_psq);
            self.nnue
                .feature_transformer
                .add_to_accumulator_psq(opp_index, opp_psq);
    ...
    pub fn update_accumulator_on_simple_move(&mut self, mov: Move) {
        ...
        // moved piece
        let my_from_index = self.nnue.feature_transformer.make_index(
            piece_moved_idx,
            1,
            from_sq_my_perspective as usize,
            my_ks as usize,
        );
        let opp_from_index = self.nnue.feature_transformer.make_index(
            piece_moved_idx,
            0,
            from_sq_opp_perspective as usize,
            opp_ks as usize,
        );
        let my_to_index = self.nnue.feature_transformer.make_index(
            piece_moved_idx,
            1,
            to_sq_my_perspective as usize,
            my_ks as usize,
        );
        let opp_to_index = self.nnue.feature_transformer.make_index(
            piece_moved_idx,
            0,
            to_sq_opp_perspective as usize,
            opp_ks as usize,
        );
        self.nnue
            .feature_transformer
            .subtract_from_accumulator(my_from_index, &mut my_acc);
        self.nnue
            .feature_transformer
            .subtract_from_accumulator(opp_from_index, &mut opp_acc);
        self.nnue
            .feature_transformer
            .add_to_accumulator(my_to_index, &mut my_acc);
        self.nnue
            .feature_transformer
            .add_to_accumulator(opp_to_index, &mut opp_acc);
        self.nnue
            .feature_transformer
            .subtract_from_accumulator_psq(my_from_index, &mut my_psq);
        self.nnue
            .feature_transformer
            .subtract_from_accumulator_psq(opp_from_index, &mut opp_psq);
        self.nnue
            .feature_transformer
            .add_to_accumulator_psq(my_to_index, &mut my_psq);
        self.nnue
            .feature_transformer
            .add_to_accumulator_psq(opp_to_index, &mut opp_psq);

        // captured piece
        if piece_captured != PieceType::Null {
            let piece_captured_idx = piece_captured as usize - 1;
            let my_captured_index = self.nnue.feature_transformer.make_index(
                piece_captured_idx,
                0,
                to_sq_my_perspective as usize,
                my_ks as usize,
            );
            let opp_captured_index = self.nnue.feature_transformer.make_index(
                piece_captured_idx,
                1,
                to_sq_opp_perspective as usize,
                opp_ks as usize,
            ...
            self.nnue
                .feature_transformer
                .subtract_from_accumulator(my_captured_index, &mut my_acc);
            self.nnue
                .feature_transformer
                .subtract_from_accumulator(opp_captured_index, &mut opp_acc);
            self.nnue
                .feature_transformer
                .subtract_from_accumulator_psq(my_captured_index, &mut my_psq);
            self.nnue
                .feature_transformer
                .subtract_from_accumulator_psq(opp_captured_index, &mut opp_psq);
        }
